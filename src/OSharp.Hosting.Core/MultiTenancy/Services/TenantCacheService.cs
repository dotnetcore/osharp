...
public class TenantCacheService : ICacheService
{
    ...
    /// <summary>
    /// 初始化一个<see cref="TenantCacheService"/>类型的新实例
    /// </summary>
    public TenantCacheService(IServiceProvider provider)
    {
        _cache = provider.GetRequiredService<IDistributedCache>();
        _tenantAccessor = provider.GetRequiredService<ITenantAccessor>();
        _logger = provider.GetLogger<TenantCacheService>();
    }
